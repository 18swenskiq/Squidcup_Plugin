...
namespace Squidcup
{
    ...
    {
        public const string warmupCfgPath = "Squidcup/warmup.cfg";
        ...
        {
            string fileName = "Squidcup/admins.json";
        ...
        {
            if (everyoneIsAdmin.Value) return true; // Everyone is treated as admin if squidcup_everyone_is_admin is true.
            ...
            if (loadedAdmins.ContainsKey(player.SteamID.ToString())) return true; // Admin exists in admins.json of Squidcup
            return false;
        }
        ...
        private void SendPausedStateMessage()
        {
            if (isPaused && matchStarted)
            {
                ...
                {
                    PrintToAllChat(Localizer["squidcup.pause.adminpausedthematch"]);
                }
                else if ((string)pauseTeamName == "RoundRestore" && !(bool)unpauseData["t"] && !(bool)unpauseData["ct"])
                {
                    PrintToAllChat(Localizer["squidcup.pause.pausedbecauserestore"]);
                }
                else if ((bool)unpauseData["t"] && !(bool)unpauseData["ct"])
                {
                    PrintToAllChat(Localizer["squidcup.pause.teamwantstounpause", reverseTeamSides["TERRORIST"].teamName, reverseTeamSides["CT"].teamName]);
                }
                else if (!(bool)unpauseData["t"] && (bool)unpauseData["ct"])
                {
                    PrintToAllChat(Localizer["squidcup.pause.teamwantstounpause", reverseTeamSides["CT"].teamName, reverseTeamSides["TERRORIST"].teamName]);
                }
                else if (!(bool)unpauseData["t"] && !(bool)unpauseData["ct"])
                {
                    PrintToAllChat(Localizer["squidcup.pause.pausedthematch", pauseTeamName]);
                }
            }
        }
        ...
        private void SendSideSelectionMessage()
        {
            ...
            PrintToAllChat(Localizer["squidcup.knife.sidedecisionpending", knifeWinnerName]);
        ...
        private void StartLive()
        {
            ...
            lastBackupFileName = $"squidcup_{liveMatchId}_{matchConfig.CurrentMapNumber}_round00.txt";
            lastSquidcupBackupFileName = $"squidcup_{liveMatchId}_{matchConfig.CurrentMapNumber}_round00.json";
        ...
        private void ResetMatch(bool warmupCfgRequired = true)
        {
            try
            {
                ...
                squidcupTeam1.teamName = "COUNTER-TERRORISTS";
                squidcupTeam2.teamName = "TERRORISTS";

                squidcupTeam1.teamPlayers = null;
                squidcupTeam2.teamPlayers = null;
                ...
                squidcupTeam1.coach = new();
                squidcupTeam2.coach = new();
                coachKillTimer?.Kill();
                coachKillTimer = null;
        ...
        private void HandleMapChangeCommand(CCSPlayerController? player, string mapName)
        {
            ...
            if (matchStarted)
            {
                ...
                ReplyToUserCommand(player, Localizer["squidcup.utility.matchstarted"]);
                return;
            }
        ...
        private void HandleReadyRequiredCommand(CCSPlayerController? player, string commandArg)
        {
            ...
            if (!string.IsNullOrWhiteSpace(commandArg))
            {
                if (int.TryParse(commandArg, out int readyRequired) && readyRequired >= 0 && readyRequired <= 32)
                {
                    ...
                    ReplyToUserCommand(player, Localizer["squidcup.utility.minreadyplayers", minimumReadyRequiredFormatted]);
                    CheckLiveRequired();
                }
                else
                {
                    ...
                    ReplyToUserCommand(player, Localizer["squidcup.utility.rrinvalidvalue"]);
                }
            }
            else
            {
                ...
                ReplyToUserCommand(player, Localizer["squidcup.utility.currentreadyrequired", minimumReadyRequiredFormatted]);
            }
        }
        ...
        private void HandleMatchStart()
        {
            ...
            if (squidcupTeam1.teamName == "COUNTER-TERRORISTS")
            {
                ...
                foreach (var key in playerData.Keys)
                {
                    ...
                    {
                        squidcupTeam1.teamName = "team_" + RemoveSpecialCharacters(playerData[key].PlayerName.Replace(" ", "_"));
            ...
            if (squidcupTeam2.teamName == "TERRORISTS")
            {
                ...
                foreach (var key in playerData.Keys)
                {
                    ...
                    {
                        squidcupTeam2.teamName = "team_" + RemoveSpecialCharacters(playerData[key].PlayerName.Replace(" ", "_"));
            ...
            {
                Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}Squidcup{ChatColors.Default} Plugin by {ChatColors.Green}Squidski{ChatColors.Default}");
            }
        ...
        private (int t1score, int t2score) GetTeamsScore()
        {
            ...
            {
                if (team.Teamname == teamSides[squidcupTeam1])
                {
                    t1score = team.Score;
                }
                else if (team.Teamname == teamSides[squidcupTeam2])
                {
                    t2score = team.Score;
                }
            }
        ...
        private void HandlePostRoundEndEvent(EventRoundEnd @event)
        {
            try
            {
                if (isMatchLive)
                {
                    ...
                    coachKillTimer = null;
                    (int t1score, int t2score) = GetTeamsScore();
                    Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}{squidcupTeam1.teamName} [{t1score} - {t2score}] {squidcupTeam2.teamName}");
                    ...
                    int ctTeamNum = reverseTeamSides["CT"] == squidcupTeam1 ? 1 : 2;
                    int tTeamNum = reverseTeamSides["TERRORIST"] == squidcupTeam1 ? 1 : 2;
                    Winner winner = new(@event.Winner.ToString(), t1score > t2score ? "team1" : "team2");

                    var roundEndEvent = new SquidcupRoundEndedEvent
                    {
                        MatchId = liveMatchId,
                        MapNumber = matchConfig.CurrentMapNumber,
                        ...
                        RoundTime = 0,
                        Winner = winner,
                        StatsTeam1 = new SquidcupStatsTeam(squidcupTeam1.id, squidcupTeam1.teamName, 0, t1score, 0, 0, playerStatsListTeam1),
                        StatsTeam2 = new SquidcupStatsTeam(squidcupTeam2.id, squidcupTeam2.teamName, 0, t2score, 0, 0, playerStatsListTeam2),
                    };
                    ...
                    lastBackupFileName = $"squidcup_{liveMatchId}_{matchConfig.CurrentMapNumber}_round{round}.txt";
                    lastSquidcupBackupFileName = $"squidcup_{liveMatchId}_{matchConfig.CurrentMapNumber}_round{round}.json";
                    Log($"[HandlePostRoundEndEvent] Setting lastBackupFileName to {lastBackupFileName} and lastSquidcupBackupFileName to {lastSquidcupBackupFileName}");
        ...
        private void PauseMatch(CCSPlayerController? player, CommandInfo? command)
        {
            if (isMatchLive && isPaused)
            {
                ...
                ReplyToUserCommand(player, Localizer["squidcup.utility.paused"]);
                return;
            }
            if (IsHalfTimePhase())
            {
                // ReplyToUserCommand(player, "You cannot use this command during halftime.");
                ReplyToUserCommand(player, Localizer["squidcup.utility.duringhalftime"]);
                return;
            }
            if (IsPostGamePhase())
            {
                // ReplyToUserCommand(player, "You cannot use this command after the game has ended.");
                ReplyToUserCommand(player, Localizer["squidcup.utility.matchended"]);
                return;
            }
            if (IsTacticalTimeoutActive())
            {
                // ReplyToUserCommand(player, "You cannot use this command when tactical timeout is active.");
                ReplyToUserCommand(player, Localizer["squidcup.utility.tacticaltimeout"]);
                return;
            }
            if (!techPauseEnabled.Value && player != null)
            {
                PrintToPlayerChat(player, Localizer["squidcup.pause.techpausenotenabled"]);
                return;
            }
        ...
        private void ForcePauseMatch(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (player == null)
            {
                Server.PrintToConsole($"[Squidcup] {Localizer["squidcup.pause.adminpausedthematch"]}");
            }
            SetMatchPausedFlags();
        }
        ...
        private void ForceUnpauseMatch(CCSPlayerController? player, CommandInfo? command)
        {
            if (matchStarted && isPaused)
            {
                ...
                PrintToAllChat(Localizer["squidcup.pause.adminunpausedthematch"]);
                UnpauseMatch();

                if (player == null)
                {
                    Server.PrintToConsole("[Squidcup] Admin has unpaused the match, resuming the match!");
                }
            }
        }
        ...
        private void ExecLiveCFG()
        {
            ...
            {
                Log($"[StartLive] Starting Live! Live CFG not found in {absolutePath}, using default CFG for gamemode: {matchConfig.GameMode}!");
                
                switch (matchConfig.GameMode.ToLower())
                {
                    case "wingman":
                    ...
                    case "3v3":
                        // 3v3 specific configuration - 20 rounds, moderate money, 35s bomb timer
                        Server.ExecuteCommand("ammo_grenade_limit_default 1;ammo_grenade_limit_flashbang 2;ammo_grenade_limit_total 4;bot_quota 0;cash_player_bomb_defused 300;cash_player_bomb_planted 300;cash_player_damage_hostage -30;cash_player_interact_with_hostage 300;cash_player_killed_enemy_default 300;cash_player_killed_enemy_factor 1;cash_player_killed_hostage -1000;cash_player_killed_teammate -300;cash_player_rescued_hostage 1000;cash_team_elimination_bomb_map 2750;cash_team_elimination_hostage_map_ct 2500;cash_team_elimination_hostage_map_t 2500;cash_team_hostage_alive 0;cash_team_hostage_interaction 600;cash_team_loser_bonus 1400;cash_team_loser_bonus_consecutive_rounds 500;cash_team_planted_bomb_but_defused 600;cash_team_rescued_hostage 600;cash_team_terrorist_win_bomb 3000;cash_team_win_by_defusing_bomb 3000;");
                        Server.ExecuteCommand("cash_team_win_by_hostage_rescue 2900;cash_team_win_by_time_running_out_bomb 2750;cash_team_win_by_time_running_out_hostage 2750;ff_damage_reduction_bullets 0.33;ff_damage_reduction_grenade 0.85;ff_damage_reduction_grenade_self 1;ff_damage_reduction_other 0.4;mp_afterroundmoney 0;mp_autokick 0;mp_autoteambalance 0;mp_backup_restore_load_autopause 1;mp_backup_round_auto 1;mp_buy_anywhere 0;mp_buy_during_immunity 0;mp_buytime 20;mp_c4timer 35;mp_ct_default_melee weapon_knife;mp_ct_default_primary \"\";mp_ct_default_secondary weapon_hkp2000;mp_death_drop_defuser 1;mp_death_drop_grenade 2;mp_death_drop_gun 1;mp_defuser_allocation 0;mp_display_kill_assists 1;mp_endmatch_votenextmap 0;mp_forcecamera 1;mp_free_armor 0;mp_freezetime 15;mp_friendlyfire 1;mp_give_player_c4 1;mp_halftime 1;mp_halftime_duration 15;mp_halftime_pausetimer 0;mp_ignore_round_win_conditions 0;mp_limitteams 0;mp_match_can_clinch 1;mp_match_end_restart 0;mp_maxmoney 16000;mp_maxrounds 20;mp_overtime_enable 1;mp_overtime_halftime_pausetimer 0;mp_overtime_maxrounds 6;mp_overtime_startmoney 10000;mp_playercashawards 1;mp_randomspawn 0;mp_respawn_immunitytime 0;mp_respawn_on_death_ct 0;mp_respawn_on_death_t 0;mp_round_restart_delay 5;mp_roundtime 1.75;mp_roundtime_defuse 1.75;mp_roundtime_hostage 1.75;mp_solid_teammates 1;mp_starting_losses 1;mp_startmoney 800;mp_t_default_melee weapon_knife;mp_t_default_primary \"\";mp_t_default_secondary weapon_glock;mp_teamcashawards 1;mp_timelimit 0;mp_weapons_allow_map_placed 1;mp_weapons_allow_zeus 1;mp_win_panel_display_time 3;spec_freeze_deathanim_time 0;spec_freeze_time 2;spec_freeze_time_lock 2;spec_replay_enable 0;sv_allow_votes 1;sv_auto_full_alltalk_during_warmup_half_end 0;sv_damage_print_enable 0;sv_deadtalk 1;sv_hibernate_postgame_delay 300;sv_ignoregrenaderadio 0;sv_infinite_ammo 0;sv_talk_enemy_dead 0;sv_talk_enemy_living 0;sv_voiceenable 1;tv_relayvoice 1;mp_team_timeout_max 3;mp_team_timeout_ot_max 1;mp_team_timeout_ot_add_each 1;mp_team_timeout_time 30;sv_vote_command_delay 0;cash_team_bonus_shorthanded 0;mp_spectators_max 20;mp_team_intro_time 0;mp_restartgame 3;mp_warmup_end;");
                        break;
                        
                    case "1v1":
                        // 1v1 specific configuration - 16 rounds, full money, 30s bomb timer, no halftime
                        Server.ExecuteCommand("ammo_grenade_limit_default 1;ammo_grenade_limit_flashbang 2;ammo_grenade_limit_total 4;bot_quota 0;cash_player_bomb_defused 300;cash_player_bomb_planted 300;cash_player_damage_hostage -30;cash_player_interact_with_hostage 300;cash_player_killed_enemy_default 300;cash_player_killed_enemy_factor 1;cash_player_killed_hostage -1000;cash_player_killed_teammate -300;cash_player_rescued_hostage 1000;cash_team_elimination_bomb_map 2500;cash_team_elimination_hostage_map_ct 2000;cash_team_elimination_hostage_map_t 2000;cash_team_hostage_alive 0;cash_team_hostage_interaction 600;cash_team_loser_bonus 1000;cash_team_loser_bonus_consecutive_rounds 300;cash_team_planted_bomb_but_defused 600;cash_team_rescued_hostage 600;cash_team_terrorist_win_bomb 2500;cash_team_win_by_defusing_bomb 2500;cash_team_win_by_hostage_rescue 2400;cash_team_win_by_time_running_out_bomb 2500;cash_team_win_by_time_running_out_hostage 2500;ff_damage_reduction_bullets 0.33;ff_damage_reduction_grenade 0.85;ff_damage_reduction_grenade_self 1;ff_damage_reduction_other 0.4;mp_afterroundmoney 0;mp_autokick 0;mp_autoteambalance 0;mp_backup_restore_load_autopause 0;mp_backup_round_auto 1;mp_buy_anywhere 0;mp_buy_during_immunity 0;mp_buytime 15;mp_c4timer 30;mp_ct_default_melee weapon_knife;mp_ct_default_primary \"\";mp_ct_default_secondary weapon_hkp2000;mp_death_drop_defuser 1;mp_death_drop_grenade 2;mp_death_drop_gun 1;mp_defuser_allocation 0;mp_display_kill_assists 1;mp_endmatch_votenextmap 0;mp_forcecamera 1;mp_free_armor 0;mp_freezetime 10;mp_friendlyfire 1;mp_give_player_c4 1;mp_halftime 0;mp_halftime_duration 15;mp_halftime_pausetimer 0;mp_ignore_round_win_conditions 0;mp_limitteams 0;mp_match_can_clinch 1;mp_match_end_restart 0;mp_maxmoney 16000;mp_maxrounds 16;mp_overtime_enable 1;mp_overtime_halftime_pausetimer 0;mp_overtime_maxrounds 4;mp_overtime_startmoney 16000;mp_playercashawards 1;mp_randomspawn 0;mp_respawn_immunitytime 0;mp_respawn_on_death_ct 0;mp_respawn_on_death_t 0;mp_round_restart_delay 3;mp_roundtime 1.5;mp_roundtime_defuse 1.5;mp_roundtime_hostage 1.5;mp_solid_teammates 1;mp_starting_losses 1;mp_startmoney 16000;mp_t_default_melee weapon_knife;mp_t_default_primary \"\";mp_t_default_secondary weapon_glock;mp_teamcashawards 1;mp_timelimit 0;mp_weapons_allow_map_placed 1;mp_weapons_allow_zeus 1;mp_win_panel_display_time 3;spec_freeze_deathanim_time 0;spec_freeze_time 2;spec_freeze_time_lock 2;spec_replay_enable 0;sv_allow_votes 0;sv_auto_full_alltalk_during_warmup_half_end 0;sv_damage_print_enable 0;sv_deadtalk 1;sv_hibernate_postgame_delay 300;sv_ignoregrenaderadio 0;sv_infinite_ammo 0;sv_talk_enemy_dead 0;sv_talk_enemy_living 0;sv_voiceenable 1;tv_relayvoice 0;mp_team_timeout_max 1;mp_team_timeout_ot_max 0;mp_team_timeout_ot_add_each 0;mp_team_timeout_time 30;sv_vote_command_delay 0;cash_team_bonus_shorthanded 0;mp_spectators_max 20;mp_team_intro_time 0;mp_restartgame 3;mp_warmup_end;");
                        break;
                        
                    case "5v5":
                    default:
                        // Default 5v5 configuration
        ...
        private void SendPlayerNotAdminMessage(CCSPlayerController? player)
        {
            ...
            ReplyToUserCommand(player, Localizer["squidcup.utility.dontpermission"]);
        }
        ...
        public void LoadClientNames()
        {
            ...
            string namesFilePath = Server.GameDirectory + "/csgo/SquidcupPlayerNames/" + namesFileName;
            ...
            WriteClientNamesInFile(sb, squidcupTeam1.teamPlayers);
            WriteClientNamesInFile(sb, squidcupTeam2.teamPlayers);
            WriteClientNamesInFile(sb, matchConfig.Spectators);

            sb.AppendLine("}");
            File.WriteAllText(namesFilePath, sb.ToString());
            Server.ExecuteCommand($"sv_load_forced_client_names_file SquidcupPlayerNames/" + namesFileName);
        }
        ...
        public string FormatCvarValue(string value)
        {
            ...
            var formattedValue = value
                ...
                .Replace("{TEAM1}", squidcupTeam1.teamName.Replace(" ", "_"))
                .Replace("{TEAM2}", squidcupTeam2.teamName.Replace(" ", "_"))
        ...
        public async Task UploadFileAsync(string? filePath, string fileUploadURL, string headerKey, string headerValue, long matchId, int mapNumber, int roundNumber)
        {
            ...
            try
            {
                ...
                content.Headers.Add("Squidcup-FileName", Path.GetFileName(filePath));
                content.Headers.Add("Squidcup-MatchId", matchId.ToString());
                content.Headers.Add("Squidcup-MapNumber", mapNumber.ToString());
                content.Headers.Add("Squidcup-RoundNumber", roundNumber.ToString());
        ...
        {
            string whitelistfileName = "Squidcup/whitelist.cfg";
